/**
 * Модальное окно для вызова врача
 * Интеграция с вебхуком Битрикса
 */
...
(function() {
    ...
    // Конфигурация
    const config = {
        webhookUrl: window.siteConfig?.bitrixWebhookUrl || 'https://amadeya.bitrix24.ru/rest/9/u37sit26v790so3z/crm.lead.add.json?FIELDS%5BNAME%5D%3D%26FIELDS%5BPHONE%5D%5B0%5D%5BVALUE%5D%3D%26FIELDS%5BOPENED%5D%3DY%26FIELDS%5BTITLE%5D%3D%25D0%2597%25D0%25B0%25D1%258F%25D0%25B2%25D0%25BA%25D0%25B0%2520%25D1%2581%2520%25D1%2581%25D0%25B0%25D0%25B9%25D1%2582%25D0%25B0%2520%25D0%2590%25D0%25BC%25D0%25B0%25D0%25B4%25D0%25B5%25D1%258F26%26FIELDS%5BASSIGNED_BY_ID%5D%3D1%26PARAMS%5BREGISTER_SONET_EVENT%5D%3DY%26FIELDS%5BSOURCE_ID%5D%3DWEB', // URL вебхука Битрикса
        privacyPolicyUrl: window.siteConfig?.privacyPolicyUrl || '/privacy-policy/', // Ссылка на политику конфиденциальности
        utmCollectorUrl: window.siteConfig?.utmCollectorUrl || '', // Тех.домен (опционально)
    ...
    // Считывание и кеширование UTM из URL
    ...
    function createModalHTML() {
        ...
        // Inline form on contacts page (if present)
    ...
    // Привязка событий
    function bindEvents() {
        // Открытие модального окна при клике на кнопку "Вызвать врача" в хедере
        ...
        // Открытие модального окна при клике на кнопку "Вызвать врача" на главной странице
        ...
        // Открытие модального окна для кнопок записи на страницах услуг
        // Модалка только для кнопок без класса booking-medflex
    ...
    // Обработка отправки inline-формы на странице контактов
    function handleInlineFormSubmit(e) {
        ...
        // Для inline-формы соблюдаем ту же логику: Bitrix GET с query, иначе POST JSON
        let inlineRequest;
        try {
            ...
            if (isBitrixLeadAdd) {
                ...
                if (!urlObj.searchParams.get('FIELDS[TITLE]')) {
                    urlObj.searchParams.set('FIELDS[TITLE]', 'Заявка с сайта');
                }
    ...
    function sendToWebhook(data) {
        ...
        // Если передан REST-хук Bitrix crm.lead.add с query-параметрами — подставим значения и отправим GET
        let requestPromise;
        try {
            const isBitrixLeadAdd = /crm\.lead\.add\.json/i.test(config.webhookUrl);
            if (isBitrixLeadAdd) {
                const urlObj = new URL(config.webhookUrl);
                // Подставляем имя и телефон
                urlObj.searchParams.set('FIELDS[NAME]', data.name || '');
                urlObj.searchParams.set('FIELDS[PHONE][0][VALUE]', data.phone || '');
                // Укажем тип телефона, чтобы Bitrix корректно принял номер
                if (!urlObj.searchParams.get('FIELDS[PHONE][0][VALUE_TYPE]')) {
                    urlObj.searchParams.set('FIELDS[PHONE][0][VALUE_TYPE]', 'MOBILE');
                }
                // Пробросим UTM-метки
                ...
                if (utm.utm_content) urlObj.searchParams.set('FIELDS[UTM_CONTENT]', utm.utm_content);
                if (utm.utm_term) urlObj.searchParams.set('FIELDS[UTM_TERM]', utm.utm_term);
                // Пробросим заголовок лида, если не задан, добавим дефолт
                if (!urlObj.searchParams.get('FIELDS[TITLE]')) {
                    urlObj.searchParams.set('FIELDS[TITLE]', 'Заявка с сайта');
                }
                requestPromise = fetch(urlObj.toString(), { method: 'GET' });
            } else {
                // Иной вебхук — отправляем JSON POST, как было
        ...
        .then(response => {
            if (response.ok) {
                // Отправим UTM-бейкон при успехе
